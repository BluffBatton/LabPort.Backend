    ...
    public class GetAlertByIdQueryHandler : IRequestHandler<GetAlertByIdQuery, AlertDto>
    {
        ...
        public async Task<AlertDto> Handle(GetAlertByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            var alert = await _context.Alerts
                ...
                .FirstOrDefaultAsync(
                    a => a.Id == request.Id && a.SensorReading.Sensor.Container.UserId == userId.Value, cancellationToken);

            if (alert == null)
            {
                throw new Exception($"Alert with id {request.Id} was not found or does not belong to current user.");
            }
